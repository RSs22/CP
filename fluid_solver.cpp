...
void set_bnd(int M, int N, int O, int b, float *x) {
  ...
  // Set corners
  x[0] = 0.33f * (x[1] + x[(M+2)] + x[IX(0, 0, 1)]);
  x[IX(M + 1, 0, 0)] =
        ...
        for (int ii = 1; ii <= M; ii += blockSize) {
          for (int k = kk; k < std::min(kk + blockSize, O + 1); k++) {
            ...
            for (int j = jj; j < std::min(jj + blockSize, N + 1); j++) {
              for (int i = ii; i < std::min(ii + blockSize, M + 1); i++) {
                vjei = i + mm2 * j;
                vjei_k = vjei + vk;
                x[vjei_k] = (x0[vjei_k] +
                            a * (x[vjei_k-1] + x[vjei_k+1] +
                                 x[vjei_k-mm2] + x[vjei_k+mm2] +
                                 x[vjei_k-mm2nm2] + x[vjei_k+mm2nm2])) * invC;
              }
            }
          }
        }
...
void project(int M, int N, int O, float *u, float *v, float *w, float *p,
             float *div) {
  float invmax = 1.0f / MAX(M, MAX(N, O));
  for (int k= 1; k <= O; k++) {
    for (int j = 1; j <= N; j++) {
      for (int i = 1; i <= M; i++) {
        div[IX(i, j, k)] =
            ...
            (u[IX(i + 1, j, k)] - u[IX(i - 1, j, k)] + v[IX(i, j + 1, k)] -
             v[IX(i, j - 1, k)] + w[IX(i, j, k + 1)] - w[IX(i, j, k - 1)]) * invmax;
        p[IX(i, j, k)] = 0;
      }
    }
  }
